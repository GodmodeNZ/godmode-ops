...
A PostgreSQL 17 service is configured in `.github/workflows/ci.yml`. Compilation is checked for both API and frontend. Browser interaction tests and the Windows launcher require the corresponding environment; a passing TypeScript build is not a claim that those have been exercised.
