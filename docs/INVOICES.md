...
The mailbox integration only reads messages. It does not send, move, delete or mark emails as read. After connecting, enable **Automatically import supplier invoices every 15 minutes**. The scheduler runs only while this ERP server is running and starts checking within one minute. It accepts PDF, CSV and TXT attachments only, from sender addresses saved under Suppliers or explicitly entered in mailbox settings. Use a dedicated Gmail Invoices label to exclude unrelated supplier attachments; create a Gmail filter to apply that label to invoice emails. Empty supplier lists block automatic imports. Automatic imports create reviewable drafts and never approve invoices or receive stock.

The schedule and last result are persisted across restarts. Each batch has at most 50 messages; when more remain, the next batch runs after one minute. Failed pages retry after 15 minutes and deduplication avoids repeated drafts. Last import counts and errors appear in Invoices; uncheck the box to pause. Run one mailbox scheduler per ERP database. Gmail requires a registered Google OAuth web application (Gmail API enabled, client ID/secret, and the exact localhost/HTTPS redirect shown in Configure email) and browser consent. Google testing-mode consent may require periodic reauthorisation. Live Gmail consent has not been completed or tested from the development environment.
