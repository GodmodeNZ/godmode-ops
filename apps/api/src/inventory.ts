...
export async function registerInventory(app: FastifyInstance, db: PrismaClient) {
  ...
  app.patch('/skus/:id', async q => {
    const { id } = q.params as { id: string }; const b = z.object({ name: text.optional(), active: z.boolean().optional(), reorderPoint: z.number().int().nonnegative().optional() }).parse(q.body);
    return mutate(db, q, 'Update SKU', async tx => { const sku = await tx.sku.findUniqueOrThrow({ where: { id } }); return tx.sku.update({ where: { id }, data: { name: b.name, active: b.active, attributes: b.reorderPoint === undefined ? undefined : { ...(sku.attributes as object ?? {}), reorderPoint: b.reorderPoint } } }); });
  });
