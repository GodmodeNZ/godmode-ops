...
export async function registerComponentReview(app:FastifyInstance,db:PrismaClient){
 ...
 app.get('/matching/review',async()=>{
  ...
  return {items,skus:ctx.skus,variants:ctx.variants.map(v=>({...v,finished:finishedPc(v)||mappings.some(m=>m.shopDomain===v.shopDomain&&(m.shopifyVariantId===v.variantId.split('/').pop()||(!m.shopifyVariantId&&m.shopifyProductId===v.productId.split('/').pop())))})),products,progress:{total:items.length,confirmed:items.filter(x=>x.confirmed).length,unresolved:items.filter(x=>!x.confirmed).length,counted:candidates.filter(c=>c.counted).length},catalogueSyncedAt:ctx.variants.map(v=>v.syncedAt).sort().at(-1)};
 });
