...
import {catalogueCandidates,finishedPc,specificationConflicts,referenceSuggestions} from './component-evidence.js';
...
export async function registerComponentReview(app:FastifyInstance,db:PrismaClient){
 ...
 app.get('/matching/review',async()=>{
  ...
  const variantReferences=ctx.variants.filter(v=>v.present&&v.status!=='ARCHIVED'&&!finishedPc(v)&&!mappings.some(m=>m.shopDomain===v.shopDomain&&(m.shopifyVariantId===v.variantId.split('/').pop()||(!m.shopifyVariantId&&m.shopifyProductId===v.productId.split('/').pop())))).map(v=>({id:v.id,code:v.shopifySku,barcode:v.barcode,name:v.productTitle+' '+v.variantTitle}));
  ...
  const reviewItems=items.map(r=>{const source={description:r.description,code:r.code,barcode:(lines.find(l=>l.id===(r as any).lineId))?.barcode};const spreadsheet=(r as any).lineId?referenceSuggestions(source,candidates):[];return {...r,candidates:r.candidates.length?r.candidates:spreadsheet.map(s=>({...candidates.find(c=>c.id===s.id),associationEvidence:s.evidence,associationConflicts:s.conflicts})),shopifySuggestions:referenceSuggestions(source,variantReferences).map(s=>({...ctx.variants.find(v=>v.id===s.id),evidence:s.evidence,conflicts:s.conflicts,score:s.score}))};});
  return {items:reviewItems,skus:ctx.skus,families:await db.componentFamily.findMany({orderBy:{name:'asc'}}),variants:ctx.variants.map(v=>({...v,finished:finishedPc(v)||mappings.some(m=>m.shopDomain===v.shopDomain&&(m.shopifyVariantId===v.variantId.split('/').pop()||(!m.shopifyVariantId&&m.shopifyProductId===v.productId.split('/').pop())))})),products,progress:{total:items.length,confirmed:items.filter(x=>x.confirmed).length,unresolved:items.filter(x=>!x.confirmed).length,counted:candidates.filter(c=>c.counted).length},catalogueSyncedAt:ctx.variants.map(v=>v.syncedAt).sort().at(-1)};
 });
