...
try{
  // Keep npm inside this helper directory instead of the enclosing ERP workspace.
  const helperPackage=resolve(dir,'package.json');
  if(!existsSync(helperPackage))writeFileSync(helperPackage,JSON.stringify({name:'godmode-shopify-helper',private:true},null,2));
  if(!existsSync(cli)) {console.log('Installing the Shopify sign-in helper…');const r=spawnSync('npm',['install','--prefix','.','--workspaces=false','--no-audit','--no-fund','@shopify/cli@4.7.1'],{cwd:dir,env,stdio:'inherit',shell:process.platform==='win32'});if(r.status!==0||!existsSync(cli))throw new Error('Shopify helper could not be installed. Check your internet connection and try again.');}
